...
def main():
    ...
    # Listing currency + FX to USD. Fetched here (the download step) so step 2 can
    # re-screen the universe repeatedly without touching the network -- experiments
    # sweep universe size off a single download.
    cur_map = {t: infer_currency(t) for t in close_kept.columns}
    unknown = sorted(t for t, c in cur_map.items() if c is None)
    if unknown:
        print(f"  WARNING: unknown listing currency, excluded from the universe "
              f"screen: {unknown}")
    currencies = sorted({c for c in cur_map.values() if c})
    fx = fetch_fx_rates(currencies, close_kept.index, hub="USD")
    fx.to_csv(PATHS["01_fx"])
    pd.Series(cur_map, name="currency").to_csv(PATHS["01_currency"])
