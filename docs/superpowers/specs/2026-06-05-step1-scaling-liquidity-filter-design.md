...
- `liquidity_filter(close, volume, window, pct_of_median, market_key_fn=market_key)` — compute
  `avg_dollar_volume`; group tickers by `market_key`; within each group keep those `≥ pct_of_median
  * group_median`. **Zero/degenerate median group** (group median == 0): keep that group's tickers
  with avg dollar-volume > 0 (don't drop a whole market on a zero median). Returns the kept names.
- `main()` — `load_tickers` → `download_all` → `liquidity_filter` → build returns
  (`pct_change().iloc[1:]`) on the kept Close → period-end string index → trim to window → write
  **pruned** `01_prices.csv` + `01_returns.csv`; also write `01_liquidity.csv` (kept ticker, avg
  dollar-volume, market group) for inspection. Print counts at each stage (loaded → downloaded
  valid → kept after liquidity) and step timer.
...
- Zero/NaN-volume stocks → avg dollar-volume 0 → excluded (untradeable). Degenerate zero-median
  group handled as above.
- Single-stock market group: median == itself → it passes.
...
- Outputs → `experiments/results/liquidity_calibration/`: a survival-vs-threshold table per source
  (CSV + summary text) and the excluded/kept name lists for set (B).
...
- `liquidity_filter` — relative-within-group keep/drop (two groups with different scales; a stock
  below 10% of its group median dropped, others kept), zero-median-group fallback, single-stock
  group.
